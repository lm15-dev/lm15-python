...
resp = claude.call("Question 1")   # cache write
print(f"Cache write: {resp.usage.cache_write_tokens} tokens")

resp = claude.call("Question 2")   # cache hit
print(f"Cache read: {resp.usage.cache_read_tokens} tokens")

resp = claude.call("Question 3")   # cache hit
print(f"Cache read: {resp.usage.cache_read_tokens} tokens")
```
...
resp = agent.call("Add input validation to models.py")
...
resp = lm15.call("claude-sonnet-4-5", ["Find liability clauses.", contract])
```
