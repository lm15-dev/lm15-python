...
if TYPE_CHECKING:  # the generated __init__ signatures
...
_CHAT_OVERRIDABLE: frozenset[str] = frozenset({
    ...
    "forced_tool_choice", "json_schema", "reasoning_efforts", "tool_result_media", "reasoning_off",
})
...
# What an explicit reasoning-off becomes.  "send" puts the dial's off word
# on the wire (MAP-5).  "lowest" is for a model that cannot stop reasoning
# on a server that ACCEPTS the off word and reasons anyway — a paid no-op
# with nothing on the response to say so (Together gpt-oss: reasoning hidden,
# ~60 reasoning tokens billed; DeepInfra gpt-oss: run as low; Together
# GLM-5.3: ignored — live 2026-09-26).  "lowest" sends the lowest level
# instead (the first of reasoning_efforts, else "low") and records the
# substitution (MAP-13 decision 2026-09-14 §4.2, xAI's rule).  A server that
# refuses the off word loudly needs nothing here.
OpenAIChatReasoningOff = Literal["auto", "send", "lowest"]
...
class OpenAIChatCompat:
    ...
    reasoning_off: OpenAIChatReasoningOff | None = field(default=None, kw_only=True)
    ...
    # A knob's value: a policy word, or a level list for reasoning_efforts.
    model_overrides: tuple[tuple[str, dict[str, str | tuple[str, ...]]], ...] = field(default=(), kw_only=True)

    # BEGIN generated by tools/gen_init_signatures.py: do not edit
    if TYPE_CHECKING:
        def __init__(
            self,
            instruction_role: OpenAIChatInstructionRole | None = ...,
            max_tokens_field: OpenAIChatMaxTokensField | None = ...,
            stream_usage: OpenAIChatStreamUsage | None = ...,
            tool_result_name: OpenAIToolResultName | None = ...,
            assistant_after_tool_result: OpenAIChatAssistantAfterToolResult | None = ...,
            thinking_format: OpenAIChatThinkingFormat | None = ...,
            thinking_replay: OpenAIChatThinkingReplay | None = ...,
            assistant_reasoning_content: OpenAIChatAssistantReasoningContent | None = ...,
            strict_tools: OpenAIStrictTools | None = ...,
            builtin_tools: OpenAIChatBuiltinTools | None = ...,
            cache_control: OpenAICacheControl | None = ...,
            user_field: OpenAIChatUserField | None = ...,
            forced_tool_choice: OpenAIChatForcedToolChoice | None = ...,
            json_schema: OpenAIChatJsonSchema | None = ...,
            routing: dict[str, Any] | None = ...,
            extensions: dict[str, Any] | None = ...,
            *,
            tool_result_media: ToolResultMedia | None = ...,
            reasoning_off: OpenAIChatReasoningOff | None = ...,
            token_scoring: OpenAIChatTokenScoring | None = ...,
            reasoning_efforts: OpenAIChatReasoningEfforts | None = ...,
            model_overrides: Sequence[tuple[str, Mapping[str, str | Sequence[str]]]] = ...,
        ) -> None: ...
    # END generated
    ...
    def __post_init__(self) -> None:
        ...
        _check_literal_or_none(self.reasoning_off, OpenAIChatReasoningOff, "reasoning_off")
    ...
    def for_model(self, model: str) -> "OpenAIChatCompat":
        ...
        for prefix, knobs in self.model_overrides:
            if model.startswith(prefix):
                # Each knob was checked against _CHAT_OVERRIDABLE and its own
                # literal (or tuple) at construction.
                changes: dict[str, Any] = dict(knobs)
                return dataclasses.replace(self, model_overrides=(), **changes)
        return self
...
OPENAI_CHAT_PRESETS: dict[str, OpenAIChatCompat] = {
    ...
    #
    # DeepInfra, Together AI, Fireworks AI and Parasail each serve many
    # vendors' open models behind one Chat Completions root.  One policy fits
    # the four, measured live 2026-09-26 on gpt-oss, DeepSeek V4.1, GLM-5.3,
    # Qwen3 and Llama 3.3 (lm15-contract/research/providers/<id>/README.md):
    #
    # - thinking_format="reasoning_effort": the one dial all four honour for
    #   effort; "none" is the documented off (DeepInfra reasoning.md, Together
    #   reasoning.md).  Fireworks rejects the `reasoning: {...}` object (400
    #   "Extra inputs"), so the OpenRouter shape is wrong there.  Whether a
    #   MODEL can stop reasoning is the model's business: Fireworks and
    #   Parasail refuse "none" loudly where it cannot (gpt-oss, GLM-5.3);
    #   DeepInfra and Together accept it and reason anyway on those models.
    #   Stated in each dossier; not guessed from model names here.
    # - thinking_replay="native": reasoning goes back as `reasoning_content`.
    #   A code word planted in a replayed trace was recalled through that
    #   field on every host that reads a replay at all; Fireworks rejects the
    #   alternative `reasoning` field (400), so one field serves all four.
    #   The dialect default ("as_text") would paste the trace into the
    #   visible assistant text of the next request.
    # - max_completion_tokens: honoured by all four (finish_reason "length"
    #   at the cap); stream_options.include_usage honoured by all four.
    # - cache_control="none": caching is automatic on all four; none takes the
    #   OpenAI key/retention pair in the shape this dialect sends (DeepInfra's
    #   retention is prompt_cache_options.ttl 5m|1h, a mapping not made yet),
    #   so a key or a long retention is dropped WITH a record (MAP-13).
    # - tool_result_media (MAP-10, live 2026-09-26, research/providers/
    #   media_tool_results.py): Fireworks (GLM-5.3-Flash) and Parasail
    #   (Qwen3-VL-8B) read an image inside a tool result -> "images"
    #   (documents untested, so they raise).  DeepInfra answers 422 "Input
    #   should be a valid string" on messages.2.tool.content -> "reject".
    #   Together: open cell (the account hit its credit limit during the
    #   pass; its serverless vision model is Kimi-K3) -> "reject" until a
    #   receipt.
    "deepinfra": OpenAIChatCompat(
        instruction_role="system",
        max_tokens_field="max_completion_tokens",
        stream_usage="include",
        thinking_format="reasoning_effort",
    ...
    # - a forced tool choice answers HTTP 500 "Internal server error" every
    #   time.  A 500 is classed retryable, so a caller's retry loop would
    #   spin on a request that cannot succeed: refuse it before the wire.
    # - an effort word outside low|medium|high (xhigh, max, and even
    #   "bogus") is accepted and run at the default, medium — fewer reasoning
    #   tokens than `high` for a request that asked for `max`.  Declaring the
    #   levels clamps to the nearest one with a record (MAP-13), as for
    #   Moonshot's kimi-k3.  DeepSeek on Together refuses unknown words
    #   loudly (400), and the other three hosts map xhigh/max to high or
    #   refuse, so nothing is declared host-wide.
    "together": OpenAIChatCompat(
        instruction_role="system",
        max_tokens_field="max_completion_tokens",
        stream_usage="include",
        thinking_format="reasoning_effort",
        ...
        tool_result_media="reject",
        model_overrides=(
            ("openai/gpt-oss", {"forced_tool_choice": "reject", "reasoning_efforts": ("low", "medium", "high"),
                                "reasoning_off": "lowest"}),
            ("zai-org/GLM-5.3", {"reasoning_off": "lowest"}),
        ),
    ),
    "fireworks": OpenAIChatCompat(
        instruction_role="system",
        max_tokens_field="max_completion_tokens",
        stream_usage="include",
        thinking_format="reasoning_effort",
        ...
        strict_tools="omit",
        cache_control="none",
        tool_result_media="images",
    ),
    "parasail": OpenAIChatCompat(
        instruction_role="system",
        max_tokens_field="max_completion_tokens",
        stream_usage="include",
        thinking_format="reasoning_effort",
...
OPENAI_CHAT_PRESET_BASE_URLS: dict[str, str] = {
    ...
    # docs.deepinfra.com chat--overview.md (the /v1/openai root, not /v1),
    # docs.together.ai openai-compatibility.md, docs.fireworks.ai
    # openai-compatibility.md, docs.parasail.io authentication.md.
...
class ResolvedOpenAIChatCompat:
    ...
    reasoning_off: Literal["send", "lowest"] = field(default="send", kw_only=True)
...
class AnthropicCompat:
    ...
    if TYPE_CHECKING:
        def __init__(
            self,
            thinking_format: AnthropicThinkingFormat | None = ...,
            cache_control: AnthropicCacheControl | None = ...,
            structured_output: AnthropicStructuredOutput | None = ...,
            parallel_tool_calls: AnthropicParallelToolCalls | None = ...,
            model_prefixes: Sequence[str] | None = ...,
            extensions: dict[str, Any] | None = ...,
            *,
            thinking_replay: AnthropicThinkingReplay | None = ...,
            sampling_params: AnthropicSamplingParams | None = ...,
            tool_result_media: ToolResultMedia | None = ...,
